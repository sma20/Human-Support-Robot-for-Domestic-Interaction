...
class RobotControl:
    ...
    def goToPositionInAStraightLine(self, speed, destination_x, destination_y):
        ...
        print "odometry:%f, %f" % (poseX, poseY)
   ...
   #Goes to the desired position
    def goToPosition(self, speed, destination_x, destination_y):
        ...
        prev_destination_angle = math.atan2(destination_y - poseY, destination_x - poseX)
        ...
        goal_begin.header.stamp = rospy.Time(0)
        goal_begin.header.frame_id = "map"
        goal_begin.pose.position=Point(destination_x,destination_y, 0)
        quat = quaternion_from_euler(0, 0, feed)
        goal_begin.pose.orientation = Quaternion(*quat)
        self.publish_in_cmd(goal_begin)

        while ((poseX > destination_x + self.pos_tolerance or poseX< destination_x - self.pos_tolerance) \
        or (poseY > destination_y + self.pos_tolerance or poseY < destination_y - self.pos_tolerance)) \
        and not (math.sqrt((poseX - initial_x)**2 + (poseY - initial_y)**2) > initialDistance) or (rospy.Time(0)> goal_begin.header.stamp*2):
            i=0 #waiting loop
    ...
    def publish_in_cmd(self,goal_begin):
        """
...
def getPose(msg):
    ...
    while not rospy.is_shutdown() and not exit:
        try:
            (trans, rot) = tfListener.lookupTransform('map', 'base_footprint', rospy.Time(0)) #CHECK
            call_pose()
            current_x = trans[0]
            current_y = trans[1]
